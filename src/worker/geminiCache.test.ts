...
describe("getOrCreateGeminiCache", () => {
  ...
  describe("Gemini API payload", () => {
    it("sends correct model, systemInstruction and ttl", async () => {
      ...
      await getOrCreateGeminiCache(db, "my-api-key", 1, 2, "contexto del negocio");
  ...
  describe("D1 persistence", () => {
    it("persists new cache name and expiry in D1", async () => {
      ...
      await getOrCreateGeminiCache(db, "key", 7, 42, "ctx");

      const [name, expiresAt, userId, negocioId] = updateBind.mock.calls[0] as [string, number, number, number];
    ...
    it("does not persist to D1 when Gemini API fails", async () => {
      ...
      await getOrCreateGeminiCache(db, "key", 1, 2, "ctx");
  ...
  describe("error handling", () => {
    it("returns null when Gemini API returns non-ok status", async () => {
      ...
      expect(await getOrCreateGeminiCache(db, "key", 1, 2, "ctx")).toBeNull();
    });
