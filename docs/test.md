...
Cobertura actual:
- `src/react-app/lib/api.test.ts`: verifica que `apiFetch` agregue `X-Negocio-ID` cuando corresponde, que no lo inyecte cuando `negocioId` está ausente, que dispare el evento global `USAGE_LIMIT_EVENT` cuando el backend responde `429 USAGE_LIMIT_EXCEEDED`, que un `429 TOO_MANY_REQUESTS` (rate limit de auth) **no** dispare ese evento, que inyecte `Authorization: Bearer <token>` cuando `bearer_token` está en `localStorage`, y que omita ese header cuando `bearer_token` está ausente.
...
- `src/react-app/hooks/useCompras.test.ts`: verifica el hook `useCompras` (22 tests). Cubre 7 bloques: **estado inicial** (`isLoading: true`, `compras: []`, `summary: []`, `error: null`); **fetchCompras** (compras actualizadas en éxito; error con mensaje del servidor en fallo; mensaje fallback cuando `error.message` no viene; "Error de conexión" en excepción de red; retorna sin fetch cuando `currentNegocio` es null); **fetchSummary** (summary actualizado en éxito; silencioso en error de red; retorna sin fetch cuando `currentNegocio` es null); **createCompra** (`true` en éxito; `false` en fallo; `false` sin negocio); **updateCompra** (`true`/`false` según respuesta del servidor); **deleteCompra** (`true`/`false` según respuesta del servidor); **uploadComprobante** (retorna la key en éxito; null en fallo; null sin negocio — usa native fetch con FormData); **getComprobanteUrl** (retorna `/api/compras/files/:key`).
- `src/react-app/pages/modulos/Compras.test.tsx`: verifica la página de compras. Cubre: spinner de carga; título de la página; selectores de mes y año; mensaje de error; botones "Nueva Compra" e "Historial"; montos por día en el calendario y conteo de ítems cuando hay datos de resumen. El `BASE_MOCK` incluye `getComprobanteUrl` (función añadida al hook en commit anterior).
...
- `src/react-app/context/AuthContext.test.tsx`: verifica `AuthProvider` (3 tests en 2 bloques). **Bearer token en `/api/users/me`** (2 tests): incluye el header `Authorization: Bearer <token>` en la llamada inicial cuando `bearer_token` está en `localStorage`; omite el header cuando `localStorage` está vacío. **Logout** (1 test): elimina `bearer_token` de `localStorage` al llamar a `logout()`.
...
- `src/react-app/pages/LandingPage.test.tsx`: verifica la landing page pública en 4 bloques (27 tests). Mockea `@capacitor/core` (via `importOriginal`) y `@codetrix-studio/capacitor-google-auth`. **WaveBackground** (4 tests): renderiza el elemento SVG, contiene exactamente 6 paths de onda, los paths tienen `fill="none"`, y el SVG tiene `preserveAspectRatio="xMidYMid slice"`. **Renderizado** (13 tests): branding "La Hoja", links de navegación con anclas correctas, h1 del hero, botón de login en navbar, botones "Empezá gratis" (hero + CTA), sección de pain points con su heading, las 6 tarjetas de problemas, sección de módulos con su heading, los 6 títulos de módulo, beneficios en verde por módulo, sección "¿Cómo funciona?" con los 3 pasos, las 4 estadísticas de confianza y el footer con copyright. **Autenticación** (4 tests): redirige a `/agente-ia` cuando el usuario está autenticado; no redirige mientras `isPending` es true; muestra el banner de email verificado con `?verified=true`; no muestra el banner sin el query param. **Login** (6 tests): al hacer click en el botón de login llama a `/api/oauth/google/redirect_url?platform=web` y redirige a la URL recibida; muestra "Conectando..." y spinner durante la carga; el botón queda deshabilitado mientras espera; vuelve a habilitarse si el fetch falla; muestra banner de error cuando el fetch falla; en plataforma nativa llama a `GoogleAuth.signIn()` y envía el `idToken` a `/api/sessions`.
- `src/react-app/pages/AuthCallback.test.tsx`: verifica la página de callback OAuth en 7 bloques (15 tests). **Estado inicial** (1 test): muestra spinner y texto "Procesando autenticación" mientras el fetch está pendiente. **Error** (4 tests): estado de error cuando no hay `?code` en la URL (mensaje `/No hay code en la URL/`); cuando el servidor responde `success: false`; cuando `fetch` lanza excepción de red (mensaje `/Error: network/`); cuando Google devuelve `?error=access_denied` en la URL (mensaje `/Google OAuth error: access_denied/`). **Verificación pendiente** (1 test): navega a `/verify-email` cuando el servidor responde `error.code === "PENDING_VERIFICATION"`. **Éxito** (2 tests): muestra estado de éxito tras autenticación correcta; llama a `window.location.assign("/agente-ia")` después del timeout. **Interacción** (1 test): el botón "Volver a intentar" navega a `/` desde el estado de error. **Plataforma** (2 tests): envía `platform: "android"` en el body cuando `isNativePlatform()` es true; no envía el campo cuando es web (browser desktop). **Android Chrome** (3 tests): envía `platform: "android_chrome"` cuando detecta Android en `navigator.userAgent` y no es plataforma nativa; redirige a `org.lahoja.app://session?token=...` cuando el servidor retorna `token`; no muestra estado de éxito cuando redirige al scheme de la app.
...
- `src/worker/index.test.ts`: verifica la función exportada `createSession` (2 tests): produce un JWT verificable con el payload correcto; el JWT expira en aproximadamente 30 días (±5 s). Verifica también la constante exportada `COOKIE_MAX_AGE`: equivale a 30 días en segundos (2592000).
...
- `src/react-app/App.test.tsx`: verifica el componente `DeepLinkHandler` (6 tests). **Plataforma web** (1 test): no registra el listener de `appUrlOpen` cuando `isNativePlatform()` es false. **Plataforma nativa** (3 tests): registra el listener `"appUrlOpen"` al montar; navega a la ruta extraída (`pathname + search`) cuando recibe una URL con protocolo `org.lahoja.app:`; no navega cuando el protocolo es distinto. **Deep link /session** (2 tests): guarda `bearer_token` en `localStorage` y navega a `/agente-ia` al recibir `org.lahoja.app://session?token=...`; no navega si el token está ausente en la URL.
...
- Módulos sin cobertura: `useAdmin.ts` (0%). Cobertura parcial: `AuthContext.tsx` (parcial — subió desde 1.88% con los nuevos tests en `AuthContext.test.tsx`), `src/worker/index.ts` (21.53%), `EmployeeModal.tsx` (26.47%), `Employees.tsx` (48.42%), `Settings.tsx` (32.98%), `Admin.tsx` (59.35%), `Compras.tsx` (75.3%), `Facturacion.tsx` (75%), `Salaries.tsx` (76.27% statements / 79.41% branches), `Sellers.tsx` (90.47% statements).
