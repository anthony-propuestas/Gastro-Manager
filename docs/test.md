...
- `src/react-app/App.test.tsx`: verifica el componente `DeepLinkHandler` (4 tests). **Plataforma web** (1 test): no registra el listener de `appUrlOpen` cuando `isNativePlatform()` es false. **Plataforma nativa** (3 tests): registra el listener `"appUrlOpen"` al montar; navega a la ruta extraída (`pathname + search`) cuando recibe una URL con protocolo `org.lahoja.app:`; no navega cuando el protocolo es distinto.
