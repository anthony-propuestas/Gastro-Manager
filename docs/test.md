...
- `src/react-app/components/auth/ProtectedRoute.test.tsx`: verifica loading y redirecciones de rutas protegidas. Usuarios no autenticados son redirigidos a `/` (landing); gerentes con módulo restringido son redirigidos a `/dashboard`.
- `src/react-app/pages/LandingPage.test.tsx`: verifica la landing page pública en 5 bloques (28 tests). **WaveBackground** (3 tests): renderiza el SVG, contiene los 6 paths de onda, y tiene los atributos de preserveAspectRatio y viewBox correctos. **Renderizado general** (7 tests): muestra el branding, los links de navegación, los headings de sección, los botones de CTA y el footer. **Autenticación** (6 tests): si el usuario ya está autenticado redirige automáticamente a `/dashboard`; maneja el estado pendiente de autenticación sin redirigir prematuramente. **Flujo de login** (12 tests): al hacer click en "Continuar con Google" llama a la API, deshabilita el botón durante el loading, muestra spinner, redirige a la URL de Google en éxito, muestra error en fallo de red y restaura el botón habilitado tras el error.
...
- `src/react-app/pages/Admin.test.tsx`: verifica el panel de administración en cuatro bloques. El `BASE_MOCK` incluye `suscripciones`, `fetchSuscripciones` y `fetchPagosUsuario` (requeridos por la sección de suscripciones del panel Admin):
  ...
  - **Programa de Referidos** (18 tests): título "Programa de Referidos" visible; tabs "Referidos" y "Vendedores"; estado vacío "No hay referidos" y "No hay vendedores activos"; display de email del vendedor y del referido; badges de estado (pendiente = azul, confirmado = verde); botones "Marcar pagada" y "Marcar procesado" visibles cuando la comisión/reembolso no está pagado; cada botón llama al handler correcto con el id del referido; badges "Pagada" y "Procesado" reemplazan a los botones cuando ya están marcados; tabla de vendedores con nombre, código, stats (referidos, confirmados, comisión); llamadas a `fetchSellers()` y `fetchReferidos()` en el montaje inicial del panel admin.
...
- Módulos con cobertura 100%: `functions/[[route]].ts`, `src/worker/validation.ts`, `src/worker/rateLimitAuth.ts`, `useSellers.ts`, `useSuscripcion.ts`, `useChat.ts`, `api.ts`, `utils.ts`, `UsageBanner.tsx`, `ProtectedRoute.tsx` y todos los componentes UI.
