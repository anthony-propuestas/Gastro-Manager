...
- `src/react-app/context/ChatContext.test.tsx`: verifica el contexto del chat (2 tests). Cubre: error lanzado al llamar `useChatContext()` fuera de `ChatProvider`; y que dentro del provider el contexto expone `messages` (array), `sendMessage` (función) y `clearMessages` (función).
...
- `src/worker/chatContext.test.ts`: verifica el endpoint `POST /api/chat` con foco en la construcción de contexto (2 tests). **Queries de contexto con LIMIT:** cuando el caché D1 está expirado, confirma que `db.batch` se llama exactamente una vez y que las queries SQL incluyen `ORDER BY is_active DESC LIMIT 30` (empleados), `ORDER BY event_date ASC LIMIT 20` (eventos) y `ORDER BY t.due_date ASC LIMIT 15` (temas). **Truncado de historial:** con un historial de 10 mensajes, verifica que el payload enviado a Gemini contiene exactamente 6 items en `contents` (5 mensajes del historial truncado por `slice(-5)` más el mensaje nuevo).
