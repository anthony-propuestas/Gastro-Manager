...
- `src/worker/scheduledCleanup.test.ts`: verifica el handler `scheduled` del worker (2 tests). Cubre: que al ejecutar el cron se consultan las compras con `expires_at <= datetime('now')`, se llama a `R2_BUCKET.delete` por cada fila encontrada y se ejecuta `UPDATE compras SET comprobante_key = NULL`; y que si no hay filas expiradas no se llama a R2.
